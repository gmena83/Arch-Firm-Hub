...
export function ImportsPanel() {
  ...
  // Defaults are localized at mount time via the `t()` helper. They seed
  // editable form fields, so we don't try to re-translate after mount —
  // the user may have already typed their own text. Switch the language
  // before opening the Imports panel for a Spanish-only seed.
  const [tplName, setTplName] = useState(() => t("KONTi Standard v2", "KONTi Estándar v2"));
  const [tplColumns, setTplColumns] = useState(() => t(
    "Category, Item, Qty, Unit, Unit Price, Total",
    "Categoría, Material, Cant., Unidad, Precio Unitario, Total",
  ));
  const [tplHeader, setTplHeader] = useState(() => t(
    "KONTi Design | Build Studio\nProject Estimate Report\nConfidential",
    "KONTi Design | Build Studio\nReporte de Estimado de Proyecto\nConfidencial",
  ));
  const [tplFooter, setTplFooter] = useState(() => t(
