...
export function ImportsPanel() {
  ...
  // text never gets it overwritten by a language switch. Submission
  // resolves "" + !dirty to the current-language default via the
  // `tplDefaults` helper below.
  const tplDefaults = (lng: "en" | "es") => ({
    name: lng === "es" ? "KONTi Estándar v2" : "KONTi Standard v2",
    columns: lng === "es"
  ...
  const [tplName, setTplName] = useState(() => tplDefaults(lang).name);
  const [tplColumns, setTplColumns] = useState(() => tplDefaults(lang).columns);
  const [tplHeader, setTplHeader] = useState(() => tplDefaults(lang).header);
  const [tplFooter, setTplFooter] = useState(() => tplDefaults(lang).footer);
